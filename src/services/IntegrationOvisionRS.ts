...
export const authOvision = async (): Promise<OvisionToken> => {
    const response = await fetch('https://ovision-rs-ebs.avtoban.ru/oauth2/token?username=ab_admin&password=Buxzyk-gusnyj-0xofso', {
        method: 'GET',
        headers: {
...
export const getOvisionData = async (data: OvisionFilter, token: string): Promise<OvisionResponse> => {
    const response = await fetch(`https://ovision-rs-ebs.avtoban.ru/api/v2/events?type[]=pass&timeRange[]=${data.dateFrom}&timeRange[]=${data.dateTo}&limit=10000`, {
        method: 'GET',
        headers: {
...
export const fetchDepartmentTree = async (
  ...
  const url = `https://ovision-rs-ebs.avtoban.ru/api/v2/departments/tree`;
...
export const getOvisionPeopleData = async (token: string): Promise<OvisionPeopleResponse> => {
    const response = await fetch(`https://ovision-rs-ebs.avtoban.ru/api/v2/objects/person?search=name:&biometricsStatus=exist&limit=10000`, {
        method: 'GET',
        headers: {
            Authorization: `Bearer ${token}`,
            'Content-Type': 'application/json',
